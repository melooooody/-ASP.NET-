...
public partial class User_MyOrder : System.Web.UI.Page
{
    ...
    protected void btnFind_Click(object sender, EventArgs e)
    {
        ...
        {
            case "未发货":
                var item = db.OrderItem.Where(m => m.OrderId == int.Parse(Session["UserId"].ToString()) && m.status == 1);
                Gv1.DataSource = item;             //引用刚才建立的数据源
                Gv1.DataBind();
                checkShop();
                break;
            case "运输中":
                var item1 = db.OrderItem.Where(m => m.OrderId == int.Parse(Session["UserId"].ToString()) && m.status == 2);
                Gv1.DataSource = item1;             //引用刚才建立的数据源
                Gv1.DataBind();
                checkShop();
                break;
            case "已签收":
                var item2 = db.OrderItem.Where(m => m.OrderId == int.Parse(Session["UserId"].ToString()) && m.status == 3);
    ...
    public void checkShop()
    {
        ...
        {
            var shop = db.Shopkeeper.Single(m => m.Id == int.Parse(Gv1.Rows[i].Cells[6].Text));
            Gv1.Rows[i].Cells[6].Text = shop.ShopName.ToString();
    ...
    protected void btnCheck_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < Gv1.Rows.Count; i++)
        {
            ...
            if (cb.Checked)
            {
                if (Gv1.Rows[i].Cells[7].Text == "未发货")
                {
                    Response.Write("<script language=javascript>alert('该商品还没发出，请您耐心等待！');</script>");
                    return;
                }
                else if(Gv1.Rows[i].Cells[7].Text == "已签收")
                {
                    Response.Write("<script language=javascript>alert('该商品已签收！');</script>");
                    return;
                }
                var r = db.OrderItem.Single(m => m.ItemId == int.Parse(Gv1.Rows[i].Cells[2].Text));
                r.status = 3;
                Response.Write("<script language=javascript>alert('商品签收成功！');</script>");
                flag++;
                db.SubmitChanges();
                showAll();
